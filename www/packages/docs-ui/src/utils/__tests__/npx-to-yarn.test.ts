...
describe("npxToYarn", () => {
  ...
  describe("edge cases", () => {
    ...
    it("should handle command with path separators", () => {
      const result = npxToYarn("npx @bentoco/medusa-cli develop", "yarn")
      expect(result).toBe("yarn @bentoco/medusa-cli develop")
    })
...
npx @bentoco/medusa-cli init`
      const result = npxToYarn(multipleCommands, "yarn", true)
      expect(result).toBe(`yarn dlx create-medusa-app@latest
yarn dlx @bentoco/medusa-cli init`)
    })
