...
/**
 * The `@mikro-orm/*` subpackages that `@bentoco/framework` re-exports as of
 * v2.11.0 (`@bentoco/framework/mikro-orm/<sub>`). Imports of these can be
...
/** `awilix` is re-exported wholesale as `@bentoco/framework/awilix`. */
const AWILIX_SOURCE = "awilix"
const AWILIX_REEXPORT = "@bentoco/framework/awilix"
...
function rewriteSource(value: string): string | null {
  ...
  if (value.startsWith(MIKRO_ORM_PREFIX)) {
    ...
    if (MIKRO_ORM_REEXPORTED_SUBPATHS.has(rest)) {
      return `@bentoco/framework/mikro-orm/${rest}`
    }
  }
...
export const rule = createRule<[], MessageIds>({
  ...
  meta: {
    ...
    docs: {
      description:
        "Don't import MikroORM or awilix directly. As of Medusa v2.11, they're bundled into `@bentoco/framework`; use the `@bentoco/framework/mikro-orm/*` and `@bentoco/framework/awilix` re-exports.",
    },
    messages: {
      useFrameworkReexport:
        "Import from `{{ replacement }}` instead of `{{ source }}`. MikroORM and awilix are re-exported by `@bentoco/framework` as of Medusa v2.11.",
      noDirectMikroOrmImport:
        "Don't import directly from `{{ source }}`. As of Medusa v2.11, MikroORM is bundled into `@bentoco/framework` — use a `@bentoco/framework/mikro-orm/*` re-export.",
    },
    fixable: "code",
    schema: [],
  },
