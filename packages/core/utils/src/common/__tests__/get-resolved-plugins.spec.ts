...
describe("getResolvedPlugins | relative paths", () => {
  test("resolve configured plugins", async () => {
    await fs.createJson("node_modules/@bentoco/draft-order/package.json", {
      name: "@bentoco/draft-order",
      version: "1.0.0",
    })
    ...
    expect(plugins).toEqual(
      expect.arrayContaining([
        {
          id: "@bentoco/draft-order",
          modules: [],
          name: "@bentoco/draft-order",
          options: {},
          resolve: path.join(
            fs.basePath,
            "node_modules/@bentoco/draft-order/.medusa/server/src"
          ),
          version: "1.0.0",
        },
        {
