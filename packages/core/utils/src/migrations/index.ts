import { MikroORM, MikroORMOptions } from "@bentoco/deps/mikro-orm/core"
import {
  MigrateOptions,
  MigrationResult,
  UmzugMigration,
} from "@bentoco/deps/mikro-orm/migrations"
import {
  defineConfig,
  PostgreSqlDriver,
...
const replacements = [
  // MikroORM imports - replace mikro-orm/{subpath} with @bentoco/framework/mikro-orm/{subpath}
  {
    pattern: /from\s+['"]@?mikro-orm\/([^'"]+)['"]/g,
    // eslint-disable-next-line quotes
    replacement: 'from "@bentoco/framework/mikro-orm/$1"',
  },
  // PG imports - replace pg with @bentoco/framework/pg
  {
    pattern: /from\s+['"]pg['"]/g,
    // eslint-disable-next-line quotes
    replacement: 'from "@bentoco/framework/pg"',
  },
]
