...
`yarn add --dev @bentoco/medusa-oas-cli`

Install in the global namespace is not yet supported.
~~`npm install -g @bentoco/medusa-oas-cli`~~
...
This command will scan the `@bentoco/medusa` package in order to extract JSDoc OAS into a json file.
