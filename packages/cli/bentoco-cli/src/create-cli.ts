import { setTelemetryEnabled } from "@bentoco/telemetry"
...
/**
 * Whether `@bentoco/eslint-plugin` is installed in the consumer project.
...
function isMedusaEslintPluginInstalled(cwd: string): boolean {
  try {
    require.resolve("@bentoco/eslint-plugin", { paths: [cwd] })
    return true
  } catch {
    return false
  }
...
function addLintOptions(builder, commandLabel: string, directory: string) {
  ...
  return builder
    .option("lint", {
      type: "boolean",
      describe: `Run the Medusa linter before ${commandLabel}. Enabled by default when @bentoco/eslint-plugin is installed. Use --no-lint to skip linting.`,
...
function buildLocalCommands(cli, isLocalProject) {
  ...
  function resolveLocalCommand(command) {
    ...
    try {
      const cmdPath = resolveCwd.silent(`@bentoco/medusa/commands/${command}`)!
...
function isLocalMedusaProject() {
  ...
  try {
    ...
    // Draft order plugin can't have @bentoco/medusa as dependency,
    // so we also check for @bentoco/cli
    inMedusaProject = !!(
      (dependencies &&
        (dependencies["@bentoco/medusa"] || dependencies["@bentoco/cli"])) ||
      (devDependencies &&
        (devDependencies["@bentoco/medusa"] ||
          devDependencies["@bentoco/cli"]))
    )
  } catch (err) {
    // ignore
  }
