import { MedusaAppLoader } from "@bentoco/framework"
import { LinkLoader } from "@bentoco/framework/links"
import { MigrationScriptsMigrator } from "@bentoco/framework/migrations"
...
jest.mock("@bentoco/framework", () => ({
  MedusaAppLoader: jest.fn(),
}))

jest.mock("@bentoco/framework/links", () => ({
  LinkLoader: jest.fn(),
}))

jest.mock("@bentoco/framework/workflows", () => ({
  WorkflowLoader: jest.fn(),
}))

jest.mock("@bentoco/framework/migrations", () => ({
  MigrationScriptsMigrator: jest.fn(),
}))

jest.mock("@bentoco/framework/modules-sdk", () => ({
  MedusaModule: {
    clearInstances: jest.fn(),
  },
}))

jest.mock("@bentoco/framework/utils", () => ({
  ContainerRegistrationKeys: {
