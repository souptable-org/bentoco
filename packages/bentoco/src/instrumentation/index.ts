...
import { ApiLoader } from "@bentoco/framework/http"
import { SpanStatusCode } from "@bentoco/framework/opentelemetry/api"
import type { NodeSDKConfiguration } from "@bentoco/framework/opentelemetry/sdk-node"
import type { SpanExporter } from "@bentoco/framework/opentelemetry/sdk-trace-node"
import { TransactionOrchestrator } from "@bentoco/framework/orchestration"
import { Tracer } from "@bentoco/framework/telemetry"
import { ICachingModuleService } from "@bentoco/framework/types"
import { camelToSnakeCase, FeatureFlag } from "@bentoco/framework/utils"
...
export function instrumentHttpLayer() {
  ...
  const HTTPTracer = new Tracer("@bentoco/http", "2.0.0")
...
export function instrumentRemoteQuery() {
  const QueryTracer = new Tracer("@bentoco/query", "2.0.0")
...
export function instrumentCache() {
  ...
  const CacheTracer = new Tracer("@bentoco/caching", "2.0.0")
...
export function registerOtel(
  ...
  } = require("@bentoco/framework/opentelemetry/resources")
  const { NodeSDK } = require("@bentoco/framework/opentelemetry/sdk-node")
  const {
    SimpleSpanProcessor,
  } = require("@bentoco/framework/opentelemetry/sdk-trace-node")

  if (instrument.db) {
    const {
      PgInstrumentation,
    } = require("@bentoco/framework/opentelemetry/instrumentation-pg")
