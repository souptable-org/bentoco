...
import { authenticate, MiddlewareRoute } from "@bentoco/framework/http"
  ...
  },
  {
    ...
    middlewares: [
      async (req, res, next) => {
        ...
        try {
          const { remoteQueryObjectFromString } = await import("@bentoco/framework/utils")
          const [cartCollectionRelation] = await remoteQuery(
            remoteQueryObjectFromString({
              entryPoint: "cart_payment_collection",
              variables: { filters: { cart_id: cartId } },
              fields: ["payment_collection.id", "payment_collection.payment_sessions.provider_id"],
            })
          )
          const sessions = cartCollectionRelation?.payment_collection?.payment_sessions || []
          const hasCodSession = sessions.some((s: any) => s.provider_id === "pp_system_default")
          if (hasCodSession) {
            if (req.headers["x-cod-otp-verified"] !== "true") {
              return res.status(403).json({
                message: "Cash on Delivery requires phone verification (OTP)."
              })
            }
          }
        } catch (e) {
          // fail safe/silent or log
        }
        next()
      },
      validateAndTransformQuery(
