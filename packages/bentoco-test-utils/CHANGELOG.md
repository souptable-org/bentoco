...
- [#15887](https://github.com/medusajs/medusa/pull/15887) [`d61d8b3e0422344cb7bee091a49ecbe51d7ef50c`](https://github.com/medusajs/medusa/commit/d61d8b3e0422344cb7bee091a49ecbe51d7ef50c) Thanks [@shafi-VM](https://github.com/shafi-VM)! - fix(test-utils): replace unmaintained pg-god with @bentoco/framework/pg

  `@bentoco/test-utils` imported `pg-god` at runtime (in `database.ts` and `jest.ts`) but never declared it as a dependency. It only resolved transitively via `@bentoco/cli`, and #15635 removed `pg-god` from the CLI, so on a clean install every test using `medusaIntegrationTestRunner` failed to load with `Cannot find module 'pg-god'`.

  The two `pg-god` helpers used (`createDatabase`/`dropDatabase`) are now reimplemented natively on top of `@bentoco/framework/pg` (already a dependency), reusing the package's existing identifier-quoting and connection-termination logic. This drops the undeclared, unmaintained dependency without changing behavior.
